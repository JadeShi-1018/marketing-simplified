...
function AdSetDetailContent({ adsetId }: { adsetId: string }) {
  ...
  useEffect(() => {
    ...
    facebookApi
      .getMetaAdSetDetail(adsetId, days)
      .then((d) => active && setDetail(d))
      .catch(() => active && toast.error('Failed to load ad set detail.'))
  ...
  useEffect(() => {
    ...
    facebookApi
      .getMetaAdSetTimeseries(adsetId, days)
      .then((d) => active && setSeries(d))
      .catch(() => active && toast.error('Failed to load ad set timeseries.'))
  ...
  if (!detail) {
    return (
      <div className="mx-auto max-w-2xl py-16 text-center text-sm text-gray-500">
        Ad set not found or inaccessible.
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      <nav className="flex items-center gap-1.5 text-xs text-gray-500">
        ...
        <span className="truncate text-gray-700">
          {detail.name || detail.meta_adset_id}
        </span>
      </nav>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-gray-500">
            ...
            <span className="rounded bg-gray-100 px-1.5 py-0.5 font-mono">
              {detail.meta_adset_id}
            </span>
            {detail.optimization_goal && (
              <span className="rounded bg-gray-100 px-1.5 py-0.5">
                goal · {detail.optimization_goal}
              </span>
            )}
            {detail.billing_event && (
              <span className="rounded bg-gray-100 px-1.5 py-0.5">
                billing · {detail.billing_event}
              </span>
            )}
            <span className="rounded bg-gray-100 px-1.5 py-0.5">
              {detail.linked_ads_count} ads
            </span>
          </div>
        </div>
        ...
        <section className="space-y-5">
          ...
          <div className="rounded-xl border border-gray-200 bg-white">
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
              <h3 className="text-sm font-semibold text-gray-900">
                Ads · {detail.linked_ads_count}
              </h3>
              <p className="text-[11px] text-gray-500">
                Click creative thumbnail to see detail
              </p>
            </div>
            {detail.linked_ads.length === 0 ? (
              <div className="px-4 py-8 text-center text-xs text-gray-400">
                No ads under this ad set yet.
              </div>
            ) : (
                      ...
                      return (
                        <tr key={ad.id} className="align-top hover:bg-gray-50/60">
                            ...
                            {ad.creative ? (
                              ...
                              >
                                {thumb ? (
                                  ...
                                  <ImageIcon className="m-auto h-4 w-4 text-gray-300" />
                                )}
                              </Link>
                            ) : (
                              <div className="flex h-10 w-10 items-center justify-center rounded-md border border-dashed border-gray-200 bg-gray-50">
                                <ImageIcon className="h-4 w-4 text-gray-300" />
                              </div>
                            )}
                          </td>
                          <td className="px-4 py-2">
                            <div className="max-w-[380px] truncate text-xs font-medium text-gray-900">
                              {ad.name || ad.meta_ad_id}
                            </div>
                            <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-gray-400">
                              <span
                                className={`rounded-full px-1.5 py-0.5 font-medium ${statusBadgeClass(ad.effective_status)}`}
                              >
                                {ad.effective_status}
                              </span>
                          ...
                          <td className="px-4 py-2 text-right">
                            <span
                              className={`inline-flex rounded-md px-2 py-0.5 text-[11px] font-mono font-medium ${roasBandClass(ad.roas)}`}
                            >
                              {Number(ad.roas) > 0
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 text-[11px] text-gray-500">
            ...
            <dl className="space-y-1">
              ...
              {detail.bid_amount_cents !== null && (
                <div className="flex justify-between gap-2">
                  <dt>Bid amount</dt>
                  <dd className="font-mono text-gray-700">
                    {formatCurrency(detail.bid_amount_cents / 100, currency)}
                  </dd>
                </div>
              )}
...
function KpiCardsBlock({
  ...
  aggregates: MetaAdSetDetail['aggregates'];
  currency: string;
  days: number;
  ...
  const tiles = [
    ...
    { label: 'Hook rate', value: formatPercent(aggregates.hook_rate) },
    { label: 'Hold rate', value: formatPercent(aggregates.hold_rate) },
  ];
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
