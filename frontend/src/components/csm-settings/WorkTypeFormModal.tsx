...
interface Props {
  isOpen: boolean;
  projectId: number;
  editing: CsmWorkTypeStub | null;
  onClose: () => void;
  onSaved: (workType: CsmWorkTypeStub) => void;
}

export default function WorkTypeFormModal({
  isOpen,
  projectId,
  editing,
  onClose,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = isEdit
        ? await TicketFormAPI.updateWorkType(editing!.id, {
            name: trimmed,
            is_active: isActive,
          })
        : await TicketFormAPI.createWorkType(projectId, { name: trimmed });
      onSaved(res.data);
    } catch (err: unknown) {
      ...
      if (Object.keys(parsed).length > 0) {
        ...
        setServerError(isEdit ? 'Could not update work type.' : 'Could not create work type.');
      }
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="mx-4 w-full max-w-lg rounded-xl bg-white shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="wt-name" className="text-sm font-medium text-gray-700">
          ...
          {isEdit && (
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-gray-500">
                Inactive work types are hidden from form dropdowns.
              </p>
            </div>
          )}
