...
interface Props {
  ...
  members: Member[];
  onConverted: (tasks: TaskData[]) => void;
}
...
export default function BulkConvertActionItemDialog({
  open,
  onOpenChange,
  projectId,
  meetingId,
  ...
  const ownerOptions = [
    ...
    ...members.map((m) => ({ value: String(memberId(m)), label: memberLabel(m) })),
  ];
  ...
  const submit = async () => {
    ...
    const shared = {
      ...
      ...(ownerId !== NO_OWNER ? { owner_id: Number(ownerId) } : {}),
      ...(dueDate ? { due_date: dueDate } : {}),
    };
    try {
      const tasks = await MeetingsAPI.bulkConvertMeetingActionItemsToTasks(
        projectId,
        meetingId,
        {
          items: items.map((it) => ({ action_item_id: it.id, ...shared })),
        },
      );
      toast.success(`Converted ${tasks.length} action item(s) to tasks`);
      onConverted(tasks);
      onOpenChange(false);
    } catch (e) {
      const err = e as { response?: { data?: { detail?: string; items?: string } }; message?: string };
      const msg =
        err.response?.data?.detail ||
        err.response?.data?.items ||
        err.message ||
        'Could not bulk convert.';
      toast.error(msg);
    } finally {
      setSubmitting(false);
    }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl bg-white shadow-2xl ring-1 ring-gray-100 outline-none data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95">
          ...
          <div className="flex items-start justify-between px-5 pt-4">
            <div className="min-w-0">
              <Dialog.Title className="text-[15px] font-semibold text-gray-900">
                Convert {items.length} action item(s) to tasks
              </Dialog.Title>
              <Dialog.Description className="mt-0.5 text-xs text-gray-500">
                Shared fields applied to all selected items. All-or-nothing — one failure rolls back the batch.
              </Dialog.Description>
            </div>
          ...
          <div className="space-y-4 px-5 pb-5 pt-4">
            {items.length > 0 && (
              <ul className="max-h-32 overflow-y-auto rounded-md bg-gray-50 p-2 text-xs text-gray-600 ring-1 ring-gray-100">
                {items.map((it) => (
            ...
            <label className="flex items-center gap-2 text-xs text-gray-600">
              ...
              />
              <span>Create all as drafts (do not auto-submit)</span>
            </label>
            ...
            <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:items-center sm:justify-end">
              ...
              >
                ...
                {submitting ? 'Converting…' : `Convert ${items.length}`}
              </button>
            </div>
          </div>
        </Dialog.Content>
