...
http {
    ...
    # SMP-539 Q8: do NOT use static upstream{} blocks. A static
    # `server backend:8000` is DNS-resolved once at startup and cached, so
    # after the backend container restarts with a new IP nginx keeps proxying
    # to the dead IP and returns 502 until reloaded. The hostnames are instead
    # referenced as variables in proxy_pass below, forcing per-request re-
    # resolution via the `resolver`. Trade-off: variable proxy_pass bypasses
    # upstream keepalive pooling. (Single server each, so no failover lost.)
    # NOTE (prod/preview): infra may prefer keeping upstream{} + an nginx
    ...
    server {
        ...
        # SMP-539 Q8: hostnames as variables so proxy_pass re-resolves via the
