"""Data migration: re-encrypt LinearCredential.encrypted_access_token."""
...
def forwards(apps, schema_editor):
    ...
    LinearCredential = apps.get_model("linear_integration", "LinearCredential")

    updated = 0
    for cred in LinearCredential.objects.all():
        new_val = _reencrypt(cred.encrypted_access_token or "", active_key_id, active_fernet, legacy)
        if new_val != cred.encrypted_access_token:
            cred.encrypted_access_token = new_val
            cred.save(update_fields=["encrypted_access_token"])
            updated += 1

    logger.info("linear_integration backfill: %d rows re-encrypted.", updated)
