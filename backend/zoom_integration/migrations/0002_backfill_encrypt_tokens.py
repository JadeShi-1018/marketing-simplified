"""Data migration: re-encrypt ZoomCredential.encrypted_access_token and encrypted_refresh_token."""
...
def forwards(apps, schema_editor):
    ...
    updated = 0
    for cred in ZoomCredential.objects.all():
        changed = False
        for field in ("encrypted_access_token", "encrypted_refresh_token"):
            old = getattr(cred, field) or ""
            new = _reencrypt(old, active_key_id, active_fernet, legacy)
            if new != old:
                setattr(cred, field, new)
                changed = True
        if changed:
            cred.save(update_fields=["encrypted_access_token", "encrypted_refresh_token"])
            updated += 1

    logger.info("zoom_integration backfill: %d rows re-encrypted.", updated)
