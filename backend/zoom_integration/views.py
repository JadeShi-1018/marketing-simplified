...
class ZoomCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if not code:
            ...
            return redirect(f"{settings.FRONTEND_URL}/integrations?zoom_error={error}")
        ...
        try:
            ...
            return redirect(f"{settings.FRONTEND_URL}/integrations?zoom_error=user_not_found")
        except Exception:
            return redirect(f"{settings.FRONTEND_URL}/integrations?zoom_error=token_exchange_failed")
